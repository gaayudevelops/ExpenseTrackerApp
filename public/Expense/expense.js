...
async function SaveToBackend(event) {
    ...
    try {
        ...
        const response = await axios.post("http://52.206.219.160:3000/expense/add-expense", obj, { headers: { "Authorization": token } })
...
async function showExpense(page) {
    ...
    try{
        ...
        const response = await axios.get(`http://52.206.219.160:3000/expense/getpageexpenses?page=${page}&pageSize=${pageSize}`, 
        ...
        response.data.allExpenses.forEach((expData) => {
            ...
            deleteButton.onclick = async () => {
                ...
                const res = await axios.delete(`http://52.206.219.160:3000/expense/delete-expense/${expData.id}`, {headers: { "Authorization": token } });
                }catch (err) {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if(!token){ // a stranger accessing the app via url  
        window.location.href="http://52.206.219.160:3000/SignUp/signup.html";
    }
...
function showLeaderboard() {
    ...
    inputElement.onclick = async () => {
        try {
            ...
            const token = localStorage.getItem('token');
            const userLeaderBoardArray = await axios.get("http://52.206.219.160:3000/premium/showleaderboard", { headers: { "Authorization": token } })
...
async function download() {
    ...
    try{
        ...
        const token = localStorage.getItem('token');
        const response = await axios.get('http://52.206.219.160:3000/expense/download', { headers: { "Authorization": token } })
...
document.getElementById('rzp-btn').onclick = async function (event) {
    ...
    try{
        const token = localStorage.getItem('token');
        const response = await axios.get("http://52.206.219.160:3000/purchase/premium-membership", { headers: { "Authorization": token } })
        ...
        const options = {
            ...
            "handler": async function (response) {
                try{
                    const res = await axios.post("http://52.206.219.160:3000/purchase/updatetransactionstatus", {
